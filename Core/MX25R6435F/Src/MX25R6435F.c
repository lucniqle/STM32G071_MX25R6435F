...
MX_Err MX_Init(void)
{
	...
	Set_Reset_Pin(GPIO_PIN_SET);
	Set_WP_Pin(GPIO_PIN_SET);
	Delay_ms(1);
...
bool MX_Available(void)
{
	...
	MX_Get_Status(&data);
	//	printf("MX_Available - Status byte: %d\r\n", data);
...
MX_Err MX_Write_Page(uint8_t *pData, uint32_t address, uint32_t size)
{
	...
	uint32_t current_size = MX25R6435F_PAGE_SIZE - (address % MX25R6435F_PAGE_SIZE);
...
MX_Err MX_Write_Bytes(uint8_t *pData, uint32_t address, uint32_t size)
{
	...
	do
	{
		...
		uint8_t addr[3] = {(current_addr >> 16) & 0xFF, (current_addr >> 8) & 0xFF, current_addr & 0xFF};
		if (SPI_Send(addr, 3, 1000) != HAL_OK)
		{
			printf("Error sending write bytes addr to MX\r\n");
			Set_CS_Pin(GPIO_PIN_SET);
...
/**
 * @brief Set RESET pin
 * @param State: SET / RESET
 * @retval None
 */
void Set_Reset_Pin(GPIO_PinState State)
{
	HAL_GPIO_WritePin(RST_MX_GPIO_Port, RST_MX_Pin, State);
	Delay_ms(1);
}
...
{
	HAL_GPIO_WritePin(CS_MX_GPIO_Port, CS_MX_Pin, State);
	Delay_ms(1);
}
...
{
	HAL_GPIO_WritePin(WP_MX_GPIO_Port, WP_MX_Pin, State);
	Delay_ms(1);
}
